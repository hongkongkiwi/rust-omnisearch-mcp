...
use crate::{
    common::{
        ...
        rate_limiter::get_limiter_stats,
    },
    config::CONFIG,
};
...
impl HealthChecker {
    ...
    pub async fn check_health(&self) -> HealthStatus {
        ...
        } else if matches!(config_check.status, CheckStatus::Warn) && matches!(overall_status, ServiceStatus::Healthy) {
        ...
        if matches!(cache_check.status, CheckStatus::Fail) {
            overall_status = ServiceStatus::Unhealthy;
        } else if matches!(cache_check.status, CheckStatus::Warn) && matches!(overall_status, ServiceStatus::Healthy) {
            overall_status = ServiceStatus::Degraded;
        }
        ...
        if matches!(metrics_check.status, CheckStatus::Warn) && matches!(overall_status, ServiceStatus::Healthy) {
            overall_status = ServiceStatus::Degraded;
        }
        ...
        if matches!(providers_check.status, CheckStatus::Fail) {
            overall_status = ServiceStatus::Unhealthy;
        } else if matches!(providers_check.status, CheckStatus::Warn) && matches!(overall_status, ServiceStatus::Healthy) {
            overall_status = ServiceStatus::Degraded;
        }
        ...
        if matches!(circuit_breaker_check.status, CheckStatus::Warn) && matches!(overall_status, ServiceStatus::Healthy) {
            overall_status = ServiceStatus::Degraded;
        }
        ...
        let (status, message) = match self.validate_configuration().await {
            Ok(issues) => {
                if issues.is_empty() {
                    ...
                    (CheckStatus::Warn, Some(format!("Configuration issues: {}", issues.join(", "))))
                }
            }
            Err(e) => (CheckStatus::Fail, Some(format!("Configuration validation failed: {}", e))),
        };
    ...
    async fn check_metrics(&self) -> HealthCheck {
        ...
        let (status, message) = if !CONFIG.metrics.enabled {
            ...
            (CheckStatus::Warn, Some("Metrics collector not properly initialized".to_string()))
        };
    ...
    async fn check_providers(&self) -> HealthCheck {
        ...
        let (status, message) = if available_providers == 0 {
            (CheckStatus::Fail, Some("No providers available".to_string()))
        } else if available_providers < 3 {
            (CheckStatus::Warn, Some(format!("Only {} providers available", available_providers)))
        } else {
            (CheckStatus::Pass, Some(format!("{} providers available", available_providers)))
        };
    ...
    async fn check_circuit_breakers(&self) -> HealthCheck {
        ...
        let (status, message) = if !CONFIG.circuit_breaker.enabled {
            (CheckStatus::Pass, Some("Circuit breakers disabled".to_string()))
        } else {
            ...
            for provider in providers {
                if let Some(stats) = get_circuit_breaker_stats(provider).await {
                    if matches!(stats.state, crate::common::circuit_breaker::CircuitState::Open) {
            ...
            if open_breakers.is_empty() {
                ...
                (CheckStatus::Warn, Some(format!("Open circuit breakers: {}", open_breakers.join(", "))))
            }
        };
...
mod tests {
    ...
    async fn test_full_health_check() {
        ...
        println!("Health status: {}", serde_json::to_string_pretty(&status).unwrap());
    }
}