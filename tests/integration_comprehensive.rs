...
use omnisearch_mcp::{
    common::{
        cache::{CacheManager, MemoryCache, get_cache_manager},
        metrics::{METRICS_COLLECTOR, record_request_metrics},
        rate_limiter::{RATE_LIMITER_MANAGER, check_rate_limit},
    ...
    config::{CONFIG, CacheConfig, CacheType},
};
...
async fn test_full_search_pipeline() -> Result<()> {
    ...
    let cache_key = CacheManager::generate_cache_key(
        "test_provider",
        &validated.query,
        &format!("limit={:?}&domains={:?}", validated.limit, validated.include_domains),
    );
    ...
    // 6. Cache the results
    cache_manager.set(&cache_key, search_results.clone()).await?;
...
async fn test_cache_performance_and_consistency() -> Result<()> {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            // Set data
            cache_clone.set(&key, data_clone.clone(), Duration::from_secs(60)).await.unwrap();
...
async fn test_input_validation_edge_cases() -> Result<()> {
    ...
    for (params, should_succeed) in test_cases {
        ...
        if should_succeed {
            assert!(result.is_ok(), "Expected validation to succeed for params: {:?}", params);
        } else {
            assert!(result.is_err(), "Expected validation to fail for params: {:?}", params);
        }
    }
...
async fn test_metrics_collection_accuracy() -> Result<()> {
    ...
    if let Some(stats) = METRICS_COLLECTOR.get_provider_stats(provider).await {
        ...
        assert!(avg_ms >= 90 && avg_ms <= 110, "Average response time was {}ms", avg_ms);
    }
...
async fn test_concurrent_operations() -> Result<()> {
    ...
    assert!(cache_size < 1000, "Cache size unexpectedly large: {}", cache_size);
...
async fn test_memory_usage_stability() -> Result<()> {
    ...
    assert!(final_cache_size <= initial_cache_size + 100, 
           "Cache size grew unexpectedly: {} -> {}", initial_cache_size, final_cache_size);
