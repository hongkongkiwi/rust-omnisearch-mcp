...
use omnisearch_mcp::{
    common::{
        types::{BaseSearchParams, SearchProvider, SearchResult},
...
fn test_provider_metadata_consistency() {
    ...
    for provider in providers {
        ...
        match name {
            "tavily" => {
                assert!(description.contains("Tavily") || description.contains("factual") || description.contains("reliable"));
            },
            "google_custom_search" => {
                assert!(description.contains("Google") || description.contains("Custom Search") || description.contains("reliable"));
            },
            "reddit" => {
                assert!(description.contains("Reddit") || description.contains("OAuth2") || description.contains("discussions"));
            },
            "duckduckgo" => {
                assert!(description.contains("DuckDuckGo") || description.contains("privacy") || description.contains("tracking"));
            },
            "baidu" => {
                assert!(description.contains("Baidu") || description.contains("China") || description.contains("SerpApi"));
            },
            "brightdata" => {
                assert!(description.contains("Bright Data") || description.contains("SERP") || description.contains("filtering"));
            },
            "exa" => {
                assert!(description.contains("Exa") || description.contains("scores") || description.contains("relevance"));
            },
            _ => {
...
fn validate_provider_error(error: &omnisearch_mcp::common::types::ProviderError, expected_provider: &str) {
    // Validate that provider errors have the required fields
    assert!(!error.message.is_empty(), "Provider error message should not be empty");
    assert_eq!(error.provider, expected_provider, 
